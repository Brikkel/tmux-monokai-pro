...
main() {
  ...
  *)
  ...
  # Window option
  if $show_powerline; then
    tmux set-window-option -g window-status-current-format "#[bg=${background},fg=${white}]${left_sep} #[fg=${background},bg=${white}]#I #W${current_flags} #[bg=${black},fg=${white}]${left_sep}"
  else
